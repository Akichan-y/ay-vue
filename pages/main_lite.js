...
import dialog_comp from '~/components/dialog.vue';
...
export default {
  ...
  mounted(){
    ...
    this.fire();
    // console.log("第４");
    this.$store.dispatch('hello2/fireStorageList')
  },
  computed: {
    ...
    getGenzaiJikan(){
      //現在までの経過時間（投入工数）
      return this.$store.getters["timeBank/getGenzaiJikan"]
  ...
  methods:{
    ...
    fire:function(){
      ...
      // ↓orderByKey では、動かない
        // .orderByKey('endTime').startAt(startTimestamp).endAt(endTimestamp);
      // とりあえずスタータスだけでも表示さると良かったが、だめみたい。
      // esp.once('value', (snapshot) => {
        
      //   let data = snapshot.val();  
      //   for (let i in data) {
      //     let person = data[i];
      //     this.$store.commit('timeBank/statusArryUD',{machineCode:person.machine,statusBool:person.status});
      //   };
      ...
      esp.on('value',(snapshot)=>{
            ...
            for(let i in data){
              ...
              // console.log("現在、" + counterN + "件目をループ中");
              let person = data[i];
              // console.log(person);
              ...
              // if(nowYYYYMMDD == TgtYYYYMMDD_Org){   
                ...
                if(TimeDeff>30000){  
                  TimeDeff = 5;
                  //2021/10/12 ０から５に変更した。
                  //０だと、DoSwitch  で除外されてしまうため、朝一がカウントされていなかった。
                  //もっと早く気づくべきであった。
                }
                ...
                const TgtMachine=person.machine;
                  ...
                  this.DoSwitch(person.status,TgtMachine,TimeDeff,TgtHStart,TgtH,TgtDStart,TgtDEnd,Tgt_HMS,Tgt_HMS_Start);
    ...
    FireStoreGet(){
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            ...
            this.$store.commit('timeBank/CycleTimeByouArrySet',{TgtMachine:doc.id,CycleTimeByou:ctByou,Bikou:doc.data().bikou,Tantousha:doc.data().tantousha}); 
      ...
      //Vuexのデータをリセットする=================================
        ...
        for (let machine of Object.keys(this.$store.getters['timeBank/getStatusFull'])) {
          // for (let machine of Object.keys(this.$store.getters['timeBank/getStopWatchArrayPrimitive'])) {
            ...
            this.$store.commit('timeBank/StopWatchSecondsArryJustRst', machine);
  ...
  DoSwitch: function (nowStatus, TgtMachine, TimeDeff, TgtHStart, TgtH, TgtDStart, TgtDEnd, Tgt_HMS, Tgt_HMS_Start) {
      //センサーの状況を読み取るswitch================================
      switch(nowStatus){
        case 'RUN1'://正常運転のスタート
            ...
            if(TimeDeff > 2 ){ //2秒以下のチャタリングのようなデータを排除する
              ...
              this.$store.commit('timeBank/sensingTimeArryJustOnlyUD',{machineCode:TgtMachine,JustSensingTime:TgtDEnd})
              
              //   =====================
              let NowTgtCycleTimeByou = this.$store.getters["timeBank/getCycleTimeByouArry"](TgtMachine);
              let BeforeStartTime = this.$store.getters["timeBank/getStartTime"](TgtMachine);
              let JustNowCycleTime = (TgtDEnd - BeforeStartTime)/1000;
              let CurrentDeff = JustNowCycleTime-NowTgtCycleTimeByou;
              // console.log()
              // if(TgtMachine=="EX124"){
                // console.log(`【${BeforeStartTime} 】--------【 ${TgtDStart}】【 ${TgtDEnd}】`)
                // console.log(`CurrentDeffの中身【${NowTgtCycleTimeByou} 】❐❐-❐❐❐【${JustNowCycleTime} 】`)
                let AbsMark = "";
                if (CurrentDeff == 0) {
                  AbsMark = "＿"
                } else if(CurrentDeff < 0) {
                  AbsMark = "▲"
                } else {
                  AbsMark = "＋"
                }
                let CurrentDeffStr = Math.abs(CurrentDeff);
                // let CurrentDeffStr = "  " +Math.abs(CurrentDeff);
                // console.log(TgtMachine+"===>"+Math.abs(CurrentDeff));
                // console.log(NowTgtCycleTimeByou);
                // console.log(Math.abs(CurrentDeff) / NowTgtCycleTimeByou);

                //Abs10000秒（16.6分）以下のみを対象とする。
                if (Math.abs(CurrentDeff) < 1000){
                  let tmp = ""

                  if(Math.abs(CurrentDeff)/NowTgtCycleTimeByou < 0.05){
                    tmp = CurrentDeffStr+"★";
                  }else {
                    tmp = CurrentDeffStr;
                  }

                  this.$store.commit('timeBank/cycleTimeDeffArrayAdd', {machine: TgtMachine, JustNowCycleDeff: "【"+this.JikanHenkan_HMS(TgtDEnd)+"】"+this.JikanHenkan(JustNowCycleTime)+ AbsMark + tmp});
                  // this.$store.commit('timeBank/cycleTimeDeffArrayAdd', {machine: TgtMachine, JustNowCycleDeff: ""+this.JikanHenkan_HMS(TgtDStart)+"" + AbsMark + CurrentDeffStr });
                }
                
                // console.log(TgtMachine + "のサイクルタイムとの差は、" + AbsMark +Math.abs(CurrentDeff) + "秒です");
                // console.log(`【${NowTgtCycleTimeByou} 】--------【 ${JustNowCycleTime}】`)

              // }
              

              // TgtDStartをアップデート
              this.$store.commit('timeBank/startTimeUD',{machineCode:TgtMachine,TgtStartD:TgtDEnd});
        ...
        case 'RUN2': //正常運転の停止
              //時間ごとのサイクル回数===========================
              // if (TgtMachine === "EX124") {
              //   console.log(`${TgtMachine}-Start->${Tgt_HMS}`)
              // }
          let NowNum = TgtHStart - 7;
          let NowDaisu = 0;
              try {
                NowDaisu = this.$store.getters['timeBank/getcycleCountMaijiArray'](TgtMachine)[(NowNum)]+1;
              } catch(error){
                // console.log(error);
              };

              this.$store.commit('timeBank/cycleCountMaijiUD',{machineCode:TgtMachine,NowNum:NowNum,NowCount:NowDaisu,Tgt_HMS:Tgt_HMS,TgtHStart:TgtHStart,Tgt_HMS_Start:Tgt_HMS_Start})
              //======================================================
...
keikaJikan: function () {
      //日付のセットを、毎秒ごとに行うこととした。
      //これまでは、起動時にセットするだけであったので、翌日に再起動の必要があった。
      //本来は、毎分とかの頻度で良いような気がするが、夜中の１２時ちょうどに見ている人がいるかもしれない、という想定      
      this.dateSet();
      // console.log(this.getSensingTimeStart);
      const ddd= new Date();
      // const ddd= new Date(this.mydate);
      const ddd2 = new Date(this.mydate);
      // const ddd2 = new Date();
      ddd2.setHours(8,0,0);
  ...
  // console.log("&&&&&&&&&"+this.GenzaiJikan);
      ...
      for (let machine of Object.keys(this.$store.getters['timeBank/getStopWatchArrayPrimitive'])) {
        ...
        const m1xJust = dJust.getUTCHours()*60 + m1;
        ...
        this.secondsValueJust =h1Just*3600 + m1Just * 60  + s1Just; //経過時間を秒で表す
        ...
        this.$store.commit("timeBank/StopWatchArryJustUD", { machineCode:tgtMachine, StopWatchSecondsTimeJust: this.secondsValueJust });
    ...
    JikanHenkan_HMS: function (NowNumber) {
      let d = new Date(NowNumber);
      let h = "0" + d.getHours();
